################################################################################
#  EPrints - MePrints - Funders Widget
################################################################################
...
################################################################################
package EPrints::Plugin::MePrints::Widget::ERIS_Funders;
...
sub new {
	...
	$self->{name} = "EPrints Profile System: Funders Widgets";
	$self->{visible} = "all";
	$self->{advertise} = 1;
	$self->{max_display} = 10;
	return $self;
...
sub render_content {
	...
	if(scalar(@data)) {
		...
		$headrow->appendChild($headcell);
		$headcell->appendChild($self->html_phrase("fundertitle"));
		$headcell = $session->make_element("th");
		...
		foreach(@data) {
            ...
            my $url = $repo_url."/cgi/search/simple?q=".$funder."&_action_search=Search&_action_search=Search&_order=bytitle&basic_srchtype=ALL&_satisfyall=ALL";
			my $link = $session->make_element("a", href=>"$url");
			$link->appendChild($session->make_text($funder));
...
sub get_funders {
	...
	my $sql = "SELECT DISTINCT funders, COUNT(funders) FROM eprint_funders WHERE eprintid IN (".$ids_string.") GROUP BY funders ORDER BY COUNT(funders) DESC;";
	my $sth = $self->{session}->get_database->prepare($sql);
	$self->{session}->get_database->execute($sth, $sql);
        while( my(@sresult) = $sth->fetchrow_array) {
            my $funder_name = $sresult[0];
            my $count = $sresult[1];
            push @data, {funders=>$funder_name, count=>$count};
        };
	return \@data;
}
1;